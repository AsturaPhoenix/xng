...
public class Repl {
  ...
  private static final Parser IDENTIFIER =
      CharacterParser.of(Character::isJavaIdentifierStart, "identifier expected")
      .seq(CharacterParser.of(Character::isJavaIdentifierPart, "identifier expected").star())
      .flatten().map(Identifier::new).trim(),
      SCOPED_IDENTIFIER = StringParser.of("::").seq(IDENTIFIER).pick(1).map(GlobalIdentifier::new)
      .or(IDENTIFIER),
      ESCAPE = anyOf("\\\""),
      STRING = CharacterParser.of('"').seq(
          noneOf("\\\"").or(CharacterParser.of('\\').seq(ESCAPE).pick(1)).star().map(
              (List<Character> chars) -> chars.stream().map(Object::toString)
              .collect(Collectors.joining())),
          CharacterParser.of('"')).pick(1).trim(),
      COMMAND_PARSER = IDENTIFIER.seq(SCOPED_IDENTIFIER.or(STRING).star());
  
  private static final Serializable
    INPUT_ID = new Identifier("input");
  
  @Getter
  private final KnowledgeBase kb;
  /**
   * Ephemeral scope for convenience while demonstrating. This is reset upon new input.
   */
  private Map<Serializable, Node> scope;
  
  private final Subject<String>
      commandOutput = PublishSubject.create(),
