...
public class LanguageBootstrap {
  ...
  public Node parse(final String string, final Node literals, final Node transforms) {
    try {
   ...
   */
  public Node eval(final String string, final Node literals, final Node transforms) {
    try {
  ...
  public void bootstrap() {
    ...
    classifyCodePoint.conjunction(process, kb.eavNode(true, false, symbol(0), kb.node(Common.codePoint)));
    ...
    // // folding.
    // //
    // // It's probably better to wait until after process to call needsLookahead.
    ...
    // {
    // val isIdentifier = new SynapticNode().conjunction(process, kb.eavNode(true,
    // false, symbol(0), identifier));
    // // Go ahead and set up the lookahead for the inhibitors now; the inhibitors
    // will
    // // be added later.
    ...
    // InvocationNode(BuiltIn.node).literal(Common.entrypoint, Bootstrap.resolve);
    ...
    // new SynapticNode().conjunction(indexer, kb.eavNode(true, false, symbol(-1),
    // literal))
    ...
    // val link = kb.new InvocationNode(BuiltIn.associate).transform(Common.prior,
    // getPriorTail)
    ...
    // // Given an object, possibly with an entrypoint, and a value expression, if
    // the
    ...
    // val mergeExpressionArgument = new SynapticNode().conjunction(mergeArgument,
    // isExpression);
    // {
    // val goc = kb.new InvocationNode(getOrCreate)
    // .inherit(Common.object)
    ...
    // InvocationNode(BuiltIn.setProperty).literal(Common.name, Common.returnValue)
    ...
    // {
    // val foldRhs = new SynapticNode().conjunction(process, kb.eavNode(true, false,
    // symbol(0), wantRhs));
    ...
    // InvocationNode(BuiltIn.node).literal(Common.entrypoint, BuiltIn.getProperty);
    ...
    // // Baking uses `backticks` to evaluate an expression at parse time and bake
    // the
    ...
    // InvocationNode(BuiltIn.node).literal(Common.entrypoint, BuiltIn.setProperty);
    ...
    // {
    // val create = kb.new InvocationNode(BuiltIn.node).literal(Common.entrypoint,
    // indirectCall);
    ...
    // isDigit.conjunction(isCodePoint, eval("eavNode(name: 'parse.isDigit, value:
    // 'true)"));
