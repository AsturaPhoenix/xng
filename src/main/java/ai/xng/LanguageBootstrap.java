...
public class LanguageBootstrap {
  ...
  private class Spawn {
    final ActionCluster.Node gated = kb.actions.new Node(() -> kb.gated.input.new Node().activate()),
        ...
        sequenceRecognition = kb.actions.new Node(() -> kb.sequenceRecognition.new Node() {
          @Override
  ...
  private class Control {
    ...
    final StmCluster stackFrame = new StmCluster(),
        ...
        cxt = new StmCluster(),
        tmp = new StmCluster();
    final BiCluster.Node invocation = kb.stateRecognition.new Node() {
      @Override
      public String toString() {
        return "invocation";
      }
        ...
        returnTo = kb.stateRecognition.new Node();
...

    {
      ...
      asSequence(doReturn)
          ...
          .thenParallel(
              ...
              kb.actions.new Node(() -> Cluster.disassociate(stackFrame, kb.stateRecognition)))
          .then(kb.gated.gate);
    }
  }
  ...
  private class StringIterator {
    ...
    {
      val iterator = kb.stateRecognition.new Node();
      codePoint = kb.stateRecognition.new Node();
      ...
      asSequence(hasNextDecoder.isTrue)
          ...
          .then(kb.actions.new Node(() -> kb.data.rxActivations().take(2).toList()
              .subscribe(data -> {
                ((DataCluster.MutableNode<Integer>) data.get(1)).setData(
                    ((Iterator<Integer>) ((DataNode) data.get(0)).getData()).next());
                System.out.println((char) (int) data.get(1).getData());
              })))
          .then(control.stackFrame.address)
          .then(iterator)
          .thenDelay(IntegrationProfile.TRANSIENT.period())
  ...
  private class RecognitionClass {
    final BiCluster.Node character = kb.stateRecognition.new Node() {
      @Override
      public void activate() {
        System.out.println(this + " " + Scheduler.global.now());
        super.activate();
      }
  ...
  private class RecognitionSequenceMemorizer {
...

    {
      invocation = kb.entrypoint.new Node() {
        ...
        @Override
        public String toString() {
          return "RecognitionSequenceMemorizer";
        }
      };
      ...
      asSequence(capture)
          .then(
              ...
              kb.actions.new Node(() -> {
                ...
                System.out.println(kb.sequenceRecognition.activations().iterator().next().getPriors());
              }));
  ...
  private class Parse {
    ...
    final BiCluster.Node constructionPointer = kb.stateRecognition.new Node(),
        writePointer = kb.stateRecognition.new Node();

    {
      ...
      asSequence(gatedInvocation.output)
          .thenDelay(IntegrationProfile.TRANSIENT.period())
          .then(
              kb.actions.new Node(() -> System.out.println(Connections.debugGraph(control.arg1, 5))),
              control.stackFrame.address,
              constructionPointer,
              spawn.gated,
      ...
      bindPrintEntrypoint.then(control.invocation);

      val bindPrint = kb.execution.new Node() {
        @Override
        public void activate() {
          System.out.println("bindPrint");
          super.activate();
        }
      };
      ...
      asSequence(bindPrint)
          ...
          .thenParallel(
              ...
              kb.actions.new Node(() -> Cluster.disassociate(control.cxt, kb.stateRecognition)))
          .thenDelay(IntegrationProfile.TRANSIENT.period())
          .then(control.stackFrame.address)
          ...
          .then(
              ...
              writePointer,
              control.arg1,
              kb.actions.new Node(() -> Cluster.associate(control.frameFieldPriors, kb.stateRecognition)))
          .thenDelay(IntegrationProfile.TRANSIENT.period())
      ...
      asSequence(returnParseFrame)
          ...
          .then(kb.actions.new Node(() -> System.out.println("from parse")))
          .then(control.doReturn);

      {
        val call = kb.stateRecognition.new Node() {
          @Override
          public void activate() {
            System.out.println(this + " " + Scheduler.global.now());
            super.activate();
  ...
  private class Eval {
...

    {
      ...
      asSequence(executeParsed.output)
          ...
          .then(kb.actions.new Node(() -> Cluster.associate(control.stackFrame, kb.stateRecognition)))
          .thenDelay(IntegrationProfile.TRANSIENT.period())
          .then(
              control.stackFrame.address,
              control.returnTo,
              returnFrom,
              kb.actions.new Node(() -> Cluster.associate(kb.stateRecognition, kb.gated.input)),
              control.execute);
      ...
      asSequence(invocation.output)
          ...
          .then(
              control.cxt.address,
              spawn.stateRecognition,
              kb.actions.new Node(() -> Cluster.associate(control.cxt, kb.stateRecognition)))
          .thenDelay(IntegrationProfile.TRANSIENT.period())
          .then(
              control.tmp.address,
              null,
          ...
          .thenParallel(control.arg1, control.tmp.address)
          .thenDelay()
          .then(kb.actions.new Node(() -> Cluster.associate(kb.stateRecognition, kb.data)))
          .thenDelay(IntegrationProfile.TRANSIENT.period())
          .then(
              control.cxt.address,
              control.returnTo,
          ...
          .then(
              null,
              kb.actions.new Node(() -> Cluster.associate(control.stackFrame, kb.stateRecognition)))
          .thenParallel(parse.invocation, kb.gated.gate);
    }
  }
  ...
  private class StringLiteralBuilder {
...

    {
      ...
      kb.actions.new Node(isParsing).conjunction(recognitionClass.character, parse.invocation);
      ...
      asSequence(end)
          ...
          .thenDelay()
          .then(kb.actions.new Node(() -> Cluster.disassociate(control.tmp, kb.stateRecognition)))
          .thenDelay(IntegrationProfile.TRANSIENT.period())
          .then(control.stackFrame.address)
          .thenParallel(control.tmp.address, parse.writePointer)
          .then(kb.actions.new Node(() -> Cluster.associate(control.tmp, kb.stateRecognition)))
          ...
          .then(
              kb.actions.new Node(
                  () -> kb.data.new FinalNode<>(builder.toString()).activate()),
              kb.actions.new Node(() -> Cluster.associate(control.frameFieldPriors, kb.data)),
      ...
      append.then(new Decoder(kb.actions, kb.data) {
        @Override
  ...
  public LanguageBootstrap(final KnowledgeBase kb) {
    ...
    asSequence(kb.inputValue.onUpdate)
        .then(
            control.stackFrame.address,
            kb.actions.new Node(() -> {
              Cluster.scalePosteriors(control.stackFrame, STACK_FACTOR);
              System.out.println("push " + Scheduler.global.now());
            }))
        .thenDelay(IntegrationProfile.TRANSIENT.period())
        .then(
            control.stackFrame.address,
            spawn.stateRecognition,
            kb.actions.new Node(() -> Cluster.associate(control.stackFrame,
                kb.stateRecognition)))
        .thenDelay(IntegrationProfile.TRANSIENT.period())
        .then(
            control.stackFrame.address,
