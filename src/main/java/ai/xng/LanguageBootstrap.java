...
public class LanguageBootstrap {
  ...
  public Node parse(final String string, final Map<String, Node> extraContext) {
    try {
      val context = kb.newContext();
      context.fatalOnExceptions();

      if (extraContext != null) {
        for (val entry : extraContext.entrySet()) {
          context.node.properties.put(kb.node(entry.getKey()), entry.getValue());
        }
      }
      ...
      val parse = kb.new InvocationNode(Bootstrap.parse).literal(Common.value, string);
      parse.activate(context);
      context.blockUntilIdle();

      val rewriter = (DeterministicNGramRewriter) context.node.properties.get(parse)
          .getValue();
   ...
   */
  public Node eval(final String string, final Map<String, Node> extraContext) {
    try {
      val parsed = parse(string, extraContext);
  ...
  private InvocationNode rewrite(final int length) {
    return kb.new InvocationNode(BuiltIn.rewrite)
        .inherit(Common.rewriter)
  ...
  public void bootstrap() {
    ...
    indexNode(parse, parseValue, "value");

    // A common symbol/attribute for subroutines that can be activated to produce an
    // evaluated value. These nodes should have expressionTail properties.
    val expression = new SynapticNode();
    indexNode(parse, expression, "expression");
    mark(expression, expression);
    mark(expression, parseValue);

    // A common symbol for nodes that are provided as-is. These are generally passed
    // as literals.
    val literal = new SynapticNode();
    indexNode(parse, literal, "literal");
    ...
    val isIdentifierStart = kb.new InvocationNode(BuiltIn.method)
        .literal(Common.javaClass, Character.class)
        .literal(Common.name, "isJavaIdentifierStart")
        .literal(kb.param(1), int.class)
    ...
    // Common lookahead logic for rules that can be inhibited by lookahead. Defaults
    // to deferring until whitespace is consumed and operators are resolved;
    // roughly, this is the local completion of tokenization, but without string
    // folding.
    //
    // It's probably better to wait until after apply to call needsLookahead.
    val needsLookahead = new SynapticNode();
    indexNode(parse, needsLookahead, "needsLookahead");
    ...
    // Bootstrap.resolve. In some cases, we could instead just set them as transform
    // members to invocations, but that can have evaluation order side effects.
    //
    ...
    {
      val isIdentifier = new SynapticNode().conjunction(apply, kb.eavNode(true, false, symbol(0), identifier));
      // Go ahead and set up the lookahead for the inhibitors now; the inhibitors will
      // be added later.
      isIdentifier.then(needsLookahead);
      resolvedIdentifier.conjunction(isIdentifier, lookaheadOk);

      val resolveCall = kb.new InvocationNode(BuiltIn.node).literal(Common.entrypoint, Bootstrap.resolve);
      ...
      kb.new InvocationNode(BuiltIn.setProperty).transform(Common.object, resolveCall)
          .literal(Common.name, Common.literal)
          .transform(Common.value, createLiteral)
          .conjunction(resolveCall, createLiteral);

      kb.new InvocationNode(BuiltIn.setProperty).transform(Common.object, createLiteral)
    ...
    indexNode(parse, indexer, "indexer");
    {
      ...
      new SynapticNode().conjunction(indexer, kb.eavNode(true, false, symbol(-1), literal))
          .then(rewrite(3, memberLiteral, value(-1)));
      new SynapticNode().conjunction(indexer, kb.eavNode(true, false, symbol(-1), expression))
    ...
    {
      val get = kb.new InvocationNode(BuiltIn.getProperty).inherit(Common.object)
          .inherit(Common.name);
      getOrCreate.then(get);
      val absent = kb.eavNode(true, false, get, null);

      get.then(kb.new InvocationNode(BuiltIn.setProperty).literal(Common.name, Common.returnValue)
          .transform(Common.value, get))
          .inhibitor(absent);

      val create = kb.new InvocationNode(BuiltIn.node).inherit(Common.entrypoint);
      create.conjunction(get, absent)
          ...
          .then(kb.new InvocationNode(BuiltIn.setProperty).literal(Common.name, Common.returnValue)
              .transform(Common.value, create));
    }
    ...
    // Given a prior and posterior both with a expressionTail, links the posterior
    // to the prior expressionTail and replaces the prior expressionTail with the
    // posterior expressionTail.
    val mergeExpression = new SynapticNode();
    indexNode(expression, mergeExpression, "merge");
    {
      val getPriorTail = kb.new InvocationNode(BuiltIn.getProperty).transform(Common.object, Common.prior)
          .literal(Common.name, expressionTail);
      mergeExpression.then(getPriorTail);

      val getPosteriorTail = kb.new InvocationNode(BuiltIn.getProperty).transform(Common.object, Common.posterior)
          .literal(Common.name, expressionTail);
      mergeExpression.then(getPosteriorTail);

      val link = kb.new InvocationNode(BuiltIn.associate).transform(Common.prior, getPriorTail)
          .inherit(Common.posterior);
      getPriorTail.then(link);
      val updatePriorTail = kb.new InvocationNode(BuiltIn.setProperty).transform(Common.object, Common.prior)
          .literal(Common.name, expressionTail)
          .transform(Common.value, getPosteriorTail);
      getPosteriorTail.then(updatePriorTail);
    }

    // Given an object, possibly with an entrypoint, and a value expression, if the
    ...
    {
      val hasEntrypoint = kb.eavNode(true, true, kb.node(Common.object), kb.node(Common.entrypoint));

      mergePrerequisite.then(kb.new InvocationNode(BuiltIn.setProperty).inherit(Common.object)
          .literal(Common.name, Common.entrypoint)
          .inherit(Common.value))
          .inhibitor(hasEntrypoint);

      val getEntrypoint = kb.new InvocationNode(BuiltIn.getProperty).inherit(Common.object)
          .literal(Common.name, Common.entrypoint)
          .conjunction(mergePrerequisite, hasEntrypoint);
      getEntrypoint.then(kb.new InvocationNode(mergeExpression).transform(Common.prior, getEntrypoint)
          .transform(Common.posterior, Common.value));
    }

    // Given a object (typically an invocation), a name, and a symbol/value pair,
    // add the value as a literal or transform property to the object under the
    // given name. If the symbol is an expression, also merge it as a prerequisite.
    val mergeArgument = new SynapticNode();
    indexNode(expression, mergeArgument, "mergeArgument");
    {
      val isExpression = kb.eavNode(true, true, kb.node(Common.symbol), expression);
      ...
      val mergeExpressionArgument = new SynapticNode().conjunction(mergeArgument, isExpression);
      {
        val goc = kb.new InvocationNode(getOrCreate)
            .inherit(Common.object)
    ...
    // Given an object expression, possibly with an entrypoint, if the entrypoint
    ...
    indexNode(expression, finalizePrerequisites, "finalize");
    {
      ...
      finalizePrerequisites.then(kb.new InvocationNode(BuiltIn.setProperty).literal(Common.name, Common.returnValue)
    ...
    // Both rvalue and lvalue members will be rewritten as "member folds", with a
    // value that is folded into. These rules handle that.
    //
    // To use, inject a memberFold symbol after the member, with a symbol property
    // on the value for the symbol that will ultimately be written.
    //
    // Does not finalize the fold.
    val memberFold = new SynapticNode();
    indexNode(parse, memberFold, "memberFold");
    {
...

      {
        ...
        rewrite(1, wantRhs, value(0))
            .conjunction(apply, lhsEav)
            .inhibitor(valueEav);

        val hasObject = new SynapticNode().conjunction(apply, lhsEav, valueEav);
      ...
      {
        val foldRhs = new SynapticNode().conjunction(apply, kb.eavNode(true, false, symbol(0), wantRhs));
        ...
        val getSymbol = kb.new InvocationNode(BuiltIn.getProperty).transform(Common.object, value(0))
    ...
    {
      val isMember = new SynapticNode().conjunction(apply,
      ...
      val getProperty = kb.new InvocationNode(BuiltIn.node).literal(Common.entrypoint, BuiltIn.getProperty);
      indexNode(rvalueMember, getProperty, "getProperty");
      rvalueMember.then(getProperty);

      getProperty.then(kb.new InvocationNode(BuiltIn.setProperty).transform(Common.object, getProperty)
          .literal(Common.name, expressionTail)
      ...
      val finalize = kb.new InvocationNode(finalizePrerequisites).transform(Common.object, value(0))
          .conjunction(apply, kb.eavNode(true, false, symbol(0), getProperty));
    ...
    indexNode(parse, bake, "bake");
    {
      ...
      val linkResult = kb.new InvocationNode(BuiltIn.associate).transform(Common.prior, getTail)
    ...
    indexNode(parse, assignment, "assignment");
    {
      ...
      lhs.conjunction(apply, new SynapticNode().conjunction(
          kb.eavNode(true, false, symbol(0), operator),
          kb.eavNode(true, false, value(0), kb.node((int) '='))));

      val setProperty = kb.new InvocationNode(BuiltIn.node).literal(Common.entrypoint, BuiltIn.setProperty);
...

      {
        ...
        rewrite(2)
            ...
            .literal(symbol(2), memberFold)
            .transform(value(2), setProperty)
            .conjunction(setProperty, kb.eavNode(true, false, symbol(-1), expression));
...

      {
        ...
        rhs.conjunction(rhsLookaheadOk, kb.eavNode(true, true, symbol(0), parseValue));

        assignment.conjunction(rhs, kb.eavNode(true, false, symbol(-1), setProperty));
    ...
    indexNode(parse, call, "call");
    {
      ...
      {
        val create = kb.new InvocationNode(BuiltIn.node).literal(Common.entrypoint, indirectCall);
      ...
      val finalize = kb.new InvocationNode(finalizePrerequisites).transform(Common.object, value(-1));
      finalize
          .conjunction(
              apply,
    ...
    {
      // For now, only expressions can become statements, but we'll add things like
      // declarations and blocks later.
      val symbolOk = new SynapticNode().disjunction(kb.eavNode(true, false, symbol(0), expression));
      indexNode(statement, symbolOk, "symbolOk");

      val lookbehind = // No previous symbol, or statement.
          new SynapticNode().disjunction(
              new SynapticNode().conjunction(apply)
                  .inhibitor(kb.eavNode(true, true, symbol(-1))),
    ...
    indexNode(parse, statementSequence, "statementSequence");
    {
    ...
    indexNode(parse, stringLiteral, "stringLiteral");
    {
      ...
      indexNode(stringLiteral, start, "start");
      {
        ...
        val newBuffer = kb.new InvocationNode(Bootstrap.newInstance).literal(Common.javaClass, StringBuilder.class);
    ...
    indexNode(parse, intLiteral, "intLiteral");
    {
      ...
      indexNode(intLiteral, start, "start");
      {
        ...
        start.getSynapse()
            .setCoefficient(kb.eavNode(true, false, symbol(-1), eval("'parse.identifier.buffer")), -1);
        start.getSynapse()
            .setCoefficient(kb.eavNode(true, false, symbol(-1), eval("'parse.stringLiteral.buffer")), -1);
      ...
      indexNode(intLiteral, end, "end");
      {
        ...
        // operator, after which we'll backtrack to here.
            ...
            .setCoefficient(kb.eavNode(true, false, symbol(1), kb.node(Common.codePoint)), -1);
