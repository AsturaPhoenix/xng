...
public class BanditTest {
  ...
  private class BinaryBandit {
    ...
    public String toString() {
      return String.format("p: %.4g, pulls: %s, observed utility: %.4g", p, pulls, (float) reward / pulls);
    }
  }
      ...
      while (consecutiveBest < CONSECUTIVE_BEST_REQUIRED) {
        ...
        if (harness.pulls > MAX_PULLS) {
          if (efficacy > .9) {
            ...
            fail(String.format("Did not converge after %d pulls with %.2f efficacy (%.2f).", harness.pulls, efficacy,
                harness.utility()));
          }
        }
      }
